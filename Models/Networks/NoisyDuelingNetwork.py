...
from Models.Networks.CustomLayer.NoisyLinearModel import NoisyLinear
#Dueling Network with Noisy layer
...
class Network(nn.Module):
	def __init__(self, config):
		...
		self.net_navigation = nn.Sequential(
			NoisyLinear(input_shape[1], hidden_size1),
			nn.ReLU()
			)
	

		self.net_value = nn.Sequential(
			NoisyLinear(hidden_size1 + conv_out_size, hidden_size2),
			nn.ReLU(),
			NoisyLinear(hidden_size2, 1)
		)
		self.net_advantage = nn.Sequential(
			NoisyLinear(hidden_size1 + conv_out_size, hidden_size2),
			nn.ReLU(),
			NoisyLinear(hidden_size2, n_actions)
		)
