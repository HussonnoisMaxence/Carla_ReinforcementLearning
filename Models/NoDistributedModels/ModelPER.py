...
class Agent:
	def __init__(self):
		...
		self.replay = ReplayBuffer.ReplayBuffer(config)
	...
	def add_transition(self, obs, action, reward, next_obs, done):
		...
		self.replay.add_experience([obs, action, self.reward_clipping(reward), next_obs, done])
	...
	def train_nn(self):
		...
		if len(self.replay.transitions) < MIN_REPLAY_SIZE:
			return		
		#Sample transitions from the minibatch 
		mini_batch, weight = self.replay.get_batch(BATCH_SIZE) # appel remote object method
